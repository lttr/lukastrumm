---
title: JavaScript collection manipulation
date: 2020-02-27
tags: ["javascript", "data-manipulation"]
---

- [Array.prototype](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array)
- [Data Wrangling with JavaScript](https://www.manning.com/books/data-wrangling-with-javascript?a_aid=datawranglingwithjavascript&a_bid=acc654f9)
- [Data-Forge](http://www.data-forge-js.com/)
- [Linq.ts](https://github.com/kutyel/linq.ts)
- [ReactiveX/IxJS](https://github.com/ReactiveX/IxJS)
- [RxJS](https://rxjs.dev/)
- [Transducers for JavaScript](https://github.com/cognitect-labs/transducers-js)
- [Transducers.js](https://jlongster.com/Transducers.js--A-JavaScript-Library-for-Transformation-of-Data)
- [Underdash](https://surma.github.io/underdash/)
- [Lodash](https://lodash.com/)
- [Composing software](https://medium.com/javascript-scene/transducers-efficient-data-processing-pipelines-in-javascript-7985330fe73d)
- [Iterators for all the collections](https://www.wix.engineering/post/breaking-chains-with-pipelines-in-modern-javascript)
